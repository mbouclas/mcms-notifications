...
class Install extends Command
{
     ...
     * @var string
     */
    protected $signature = 'notifications:install {provisionFile?} {--action=}';
    ...
    public function handle()
    {
        ...
        $command = "php artisan notifications:refreshAssets";
    ...
    {
        Event::listen('installer.notifications.run.before', function ($msg, $type = 'comment'){
            $this->{$type}($msg);
        });

        Event::listen('installer.notifications.run.after', function ($msg, $type = 'comment'){
            $this->{$type}($msg);
        });
    }
}
